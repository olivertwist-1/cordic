...
# FOLLOWED FROM AIOHTTP multipart form-data documentation
class Writer:
    def __init__(self):
...
class HTTPClient:
    ...
    async def authenticate(self) -> None:
        ...
        self.session = aiohttp.ClientSession(headers={
            ...
            "User-Agent": f"DiscordBot ({cordic.__github__}, {cordic.__version__})"}
        )
    ...
    async def multipart_handler(
            cls,
            data: Union[str, Dict],
            files: List["cordic.File"]) -> "aiohttp.MultipartWriter":
    ...
    async def request(
            ...
            files: Optional[List["cordic.File"]] = None
