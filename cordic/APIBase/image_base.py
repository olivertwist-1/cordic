...
def make_image_url(
    ...
    if hash_ is None:
        ...
        return cordic.APIBase.default_av_url.format(
            cordic.cdn_url,
            tag % 5
        )
    ...
    url = cordic.APIBase.avatar_base_url if avatar else cordic.APIBase.banner_base_url
...
def make_application_image(id_: int, hash_: str) -> Optional[str]:
    ...
    if hash_ is not None:
        return cordic.cdn_url + f"/app-icons/{id_}/{hash_}.png"
...
def make_role_icon(id_: int, icon: str) -> Optional[str]:
    ...
    if icon is not None:
        return cordic.cdn_url + f"/role-icons/{id_}/{icon}.png"