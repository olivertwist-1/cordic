...
from cordic.models.embed import Embed
from cordic.models.file import File
from cordic.utils import DataType, make_optional
from cordic.APIBase.image_base import make_application_image
...
class MessageInteraction:
    def __init__(self, data: DataType):
        ...
        self.user: Optional[cordic.User] = make_optional(
            cordic.User,
            data.get("user")
        )
...
class Application:
    def __init__(self, data: DataType):
        ...
        self.owner = cordic.utils.make_optional(
            cordic.User,
            data.get("owner", {})
        )
...
class Message:
    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        self.member: cordic.Member = cordic.make_optional(cordic.Member, data.get("member"))
        self.author: cordic.User = cordic.User(
            data["author"],
        ...
        self.edited_timestamp: Optional[datetime] = cordic.make_optional(
            datetime.fromisoformat, data.get("edited_timestamp")
        )
        self.tts: bool = data["tts"]

        self.mention_everyone: bool = data["mention_everyone"]
        self.mentions: List[cordic.User] = list(map(cordic.User, data.get("mentions", [])))
        self.mention_roles: List[int] = [int(id_) for id_ in data["mention_roles"]]

        self.mention_channels: List[cordic.ChannelMention] = list(map(
            cordic.ChannelMention,
            data.get("mention_channels", [])
        ))
        self.attachments: List[cordic.Attachment] = list(map(
            cordic.Attachment,
            data.get("attachments", [])
        ))
        self.embeds: List[cordic.Embed] = list(map(
            cordic.Embed.from_dict,
            data.get("embeds", [])
        ))
        self.reactions: List[cordic.Reaction] = list(map(
            cordic.Reaction,
            data.get("reactions", [])
        ))
        ...
        self.application: Optional[Application] = cordic.make_optional(
            Application,
    ...
    async def from_id(channel_id: int, message_id: int) -> Message:
        ...
        return await cordic.fetch_message_base(channel_id=channel_id, message_id=message_id)
    ...
    async def send(
        ...
        message_object = await cordic.create_message(
    ...
    async def delete(self, *, delay: Optional[float] = None) -> None:
        ...
        if delay is not None:
            ...
            async def delete(delay_: float):
                ...
                await cordic.delete_message(self.channel_id, self.id)
