...
class Client:
    ...
    def __init__(self, intents: int, cache_size: int = 1500):
        ...
        self.ws = cordic.websocket.DiscordWebSocket(self, intents)
        self.http: cordic.HTTPClient = cordic.HTTPClient(bot_token="placeHolder")

        # storage
        self.events: Dict[str, Dict[str, Any]] = {}

        # cache
        self.user_cache: Dict[str, cordic.User] = {}
        self.message_cache: Dict[str, cordic.Message] = {}
    ...
    async def event_handler(self, event_name: str, event_data: Dict[str, Any]):
        ...
        if event_name.lower() == "ready":
            ...
            ready_obj = cordic.ReadyEvent(data=event_data)
        ...
        if event_name.lower() == "message_create":
            ...
            message_obj = cordic.Message(data=event_data)
    ...
    def run(self, token: str) -> None:
        ...
        cordic.CurrentClient.client = self
    ...
    @staticmethod
    async def fetch_user(user_id: int) -> cordic.User:
        """Returns an user by id"""
        return await cordic.User.from_id(user_id)

    @staticmethod
    async def fetch_channel(channel_id: int) -> cordic.Channel:
        """Returns a channel by id"""
        return await cordic.Channel.from_id(channel_id)

    @staticmethod
    async def fetch_guild_roles(guild_id: int) -> List[cordic.Role]:
        """Returns a role by id"""
        return await cordic.Role.from_guild_id(guild_id)

    @staticmethod
    async def fetch_message(channel_id: int, message_id: int) -> cordic.Message:
        """Returns the Message by channel id and message id"""
        return await cordic.Message.from_id(
            channel_id=channel_id,
            message_id=message_id
        )

    @property
    async def connections(self) -> List["cordic.Connection"]:
        """
        ...
        return list(map(cordic.Connection, await rs.json()))

    @property
    async def user(self) -> cordic.User:
        """Gets the bot user"""

        rs = await self.http.request("GET", "/users/@me")
        return cordic.User(await rs.json())
